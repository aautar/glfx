...
glfx.rttTexture = null;

// Framebuffer for Render-to-Texture
glfx.rttFramebuffer = null;

// Depth buffer for Render-to-Texture
glfx.rttDepthBuffer = null;
...
glfx.initRTTTexture = function(_frameBuffer) {
    ...
    glfx.gl.texParameteri(glfx.gl.TEXTURE_2D, glfx.gl.TEXTURE_MAG_FILTER, glfx.gl.LINEAR);
    glfx.gl.texParameteri(glfx.gl.TEXTURE_2D, glfx.gl.TEXTURE_MIN_FILTER, glfx.gl.LINEAR_MIPMAP_NEAREST);
...
glfx.init = function(_canvas, _canvasWidthCSSPx, _canvasHeightCSSPx, _onInitComplete) {
    ...
    // Setup RTT
    glfx.rttFramebuffer = glfx.initRTTFrameBuffer(glfx.gl.viewportWidth, glfx.gl.viewportHeight);
    glfx.rttTexture = glfx.initRTTTexture(glfx.rttFramebuffer);
    glfx.rttDepthBuffer = glfx.initRTTDepthBuffer(glfx.rttFramebuffer);
    glfx.gl.framebufferTexture2D(glfx.gl.FRAMEBUFFER, glfx.gl.COLOR_ATTACHMENT0, glfx.gl.TEXTURE_2D, glfx.rttTexture, 0);
    glfx.gl.framebufferRenderbuffer(glfx.gl.FRAMEBUFFER, glfx.gl.DEPTH_ATTACHMENT, glfx.gl.RENDERBUFFER, glfx.rttDepthBuffer);
...
glfx.render = function(_time) {
    ...
    glfx.gl.bindTexture(glfx.gl.TEXTURE_2D, glfx.rttTexture);
    //glfx.gl.uniform1i(shaderProgram.samplerUniform, 0);    
