...
glfx.defaultScene = null;

glfx.renderAllScenes = function() { };
...
glfx.init = function(_canvas, _canvasWidthCSSPx, _canvasHeightCSSPx, _onInitComplete) {
    ...
    glfx.defaultScene = Object.create(glfx.scene).init();
    ...
    // Begin rendering
    function renderSceneStack(_time) {
        requestAnimationFrame(renderSceneStack);
        if(glfx.assetRef < 0) {
            return;
        }

        glfx.renderAllScenes(_time);
...
glfx.renderToFrameBuffer = function(_scene, _frameBuffer, _tdelta) {
    ...
    for(var i=0; i<_scene.graph.length; i++) {

        _scene.matModelView = mat4.create();
        mat4.multiply(_scene.matModelView, _scene.matModelView, _scene.matCamera);

        _scene.graph[i].update(_tdelta, _scene.graph[i]);

        if (_scene.graph[i].matTransform) {
            mat4.multiply(_scene.matModelView, _scene.matModelView, _scene.graph[i].matTransform);
        } else {

            var objpos = _scene.graph[i].position;
            var objrot = _scene.graph[i].rotation;
            var objscale = _scene.graph[i].scale;

            mat4.scale(_scene.matModelView, _scene.matModelView, objscale);
            mat4.translate(_scene.matModelView, _scene.matModelView, objpos);
            mat4.rotate(_scene.matModelView, _scene.matModelView, objrot[0], [1, 0, 0]);
            mat4.rotate(_scene.matModelView, _scene.matModelView, objrot[1], [0, 1, 0]);
            mat4.rotate(_scene.matModelView, _scene.matModelView, objrot[2], [0, 0, 1]);

        }

        _scene.graph[i].render(_tdelta, _scene.graph[i], _scene.matModelView, _scene.matPerspective);
    }
};

glfx.renderViewportQuad = function (_scene, _texture, _shaderProgram, _tdelta) {
    ...
    _scene.onPostProcessPreDraw(_tdelta);
