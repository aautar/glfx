...
glfx.shaders.createProgram = function(_shaders, _onPostLink) {
    ...
    if (!glfx.gl.getProgramParameter(shprog, glfx.gl.LINK_STATUS)) {
        glfx.echo("Could not create shader program");
        return null;
    }    
...
glfx.render = function(_time) {
    ...
    glfx.renderViewportQuad(glfx.scene.rttTexture, glfx.rttprog);
...
glfx.renderViewportQuad = function (_texture, _shaderProgram) {

    if (glfx.rttprog === null || _shaderProgram === null) {
        return false;
    }
    ...
    pMatrix = mat4.ortho(pMatrix, -1, 1, -1, 1, 0.1, -100);    
    
    var mvMatrix = mat4.create();    
    mvMatrix = mat4.lookAt(mvMatrix, vec3.clone([0,0,0]), vec3.clone([0,0,-1]), vec3.clone([0,1,0]));
